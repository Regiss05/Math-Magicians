...
it('Render Results (Display Component) | Pass Prop [69]', () => {
  render(<Display display="69" />);
  const element = screen.getByText(/69/i);
  expect(element).toBeInTheDocument();
});

it('Render Results (Display Component) | Pass Prop [100000]', () => {
  render(<Display display="100000" />);
  const element = screen.getByText(/100000/i);
  expect(element).toBeInTheDocument();
});

it('Render Results (Display Component) | Pass Prop [-9999]', () => {
